...
function potongTeks(elemen, kataYangDitampilkan) {
	...
	let teksAwal = element.textContent
		.split(" ")
		.slice(0, kataYangDitampilkan)
		.join(" ");
	let teksSisa = element.textContent
		.split(" ")
		.slice(kataYangDitampilkan)
		.join(" ");

	element.innerHTML =
		teksAwal + '<span style="display: inline-block; margin-left: 2px;"></span>';
...
function ubahWarna(index) {
  ...
  let btnModalMassage = document.getElementById("btnModalMassage" + index);
	...
	if (checkbox.checked) {
		...
		btnModalMassage.style.display = "none";
    ...
    btnModalMassage.style.display = "block";
		// btnModalMassage.removeAttribute("disabled");
...
function toggleLinkVisibility() {
	...
	for (let i = 0; i < paragraphs.length; i++) {
		...
		linkMassage.style.display = jumlahKata >= 5 ? "inline" : "none";
	}
}
